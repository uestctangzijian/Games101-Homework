...
public:
        ...
        : v0(_v0), v1(_v1), v2(_v2), m(_m)
    {
        ...
        area = crossProduct(e1, e2).norm()*0.5f; // 三角形面积：两条边的叉积/2 https://zhuanlan.zhihu.com/p/34191849
    }
    ...
    void Sample(Intersection &pos, float &pdf){ // 通过下面的公式得出三角形上的随机点，原理未知
        float x = std::sqrt(get_random_float()), y = get_random_float();
        pos.coords = v0 * (1.0f - x) + v1 * (x * (1.0f - y)) + v2 * (x * y); // v0(1-x) + v1(x(1-y)) + v2(xy)
