...
public:
    ...
    void Sample(Intersection &pos, float &pdf){ // 随机取一个方向，求这个方向在球面上对应的点
        float theta = 2.0 * M_PI * get_random_float(), phi = M_PI * get_random_float();
        Vector3f dir(std::cos(phi), std::sin(phi)*std::cos(theta), std::sin(phi)*std::sin(theta)); // dir = (cosθ，sinφcosθ，sinφsinθ)
